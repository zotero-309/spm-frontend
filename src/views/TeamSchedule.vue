  ...
  <div class="vue-root">
    <div id="primaryContainer">
      <div id="header">
        ...
        <div id="title">My Team Schedule (of my teammates)</div>
      </div>
      <div class="content">
        <section id="sideA">
...
export default {
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      try {
        const response = await axios.get(`${baseURL}/schedule/team_schedule/${staff_id}`);
        console.log("API Response:", response.data);
        data1.value = response.data;
        const data11 = response.data;

        const response2 = await axios.get(`${baseURL}/schedule/employeelist/${staff_id}`);
        data2.value = response2.data;
        const data22 = response2.data;
        ...
        scheduler.addEventListener("editDialogOpen", function (event) {
          ...
          // Navigate to employeelist page with query parameters
          router.push({
            name: 'EmployeeList',
            query: {
              date: date,
